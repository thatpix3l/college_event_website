...
CREATE TABLE SuperAdmin(
    id INTEGER PRIMARY KEY REFERENCES BaseUser(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE RsoMember (
    id INTEGER NOT NULL REFERENCES Student(id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE PublicEvent (
    id INTEGER PRIMARY KEY REFERENCES BaseEvent(id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE TaggedEvent (
    ...
    base_event INTEGER NOT NULL REFERENCES BaseEvent(id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE Comment (
    id SERIAL PRIMARY KEY,
    body VARCHAR NOT NULL,
    posted_by INTEGER REFERENCES Student(id) ON UPDATE CASCADE ON DELETE
    SET NULL,
        base_event INTEGER NOT NULL REFERENCES BaseEvent(id) ON UPDATE CASCADE ON DELETE CASCADE
);
CREATE TABLE Rating (
